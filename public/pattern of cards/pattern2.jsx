...
const Card = ({ title, subtitle, description, icon: IconComponent }) => {
  return (
    <div className="relative bg-neutral-800 rounded-lg p-6 m-4 max-w-sm flex-grow shadow-lg border border-neutral-700 hover:border-teal-400 transition-all duration-300 transform hover:-translate-y-1 card-shadow-effect">
      {/* Icon for the card */}
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans overflow-hidden relative z-0">
      ...
      <style>{`
        /* Card general subtle glow and L-shape shadow on hover */
        .card-shadow-effect {
          position: relative;
          overflow: hidden;
          /* General subtle glow initially */
          box-shadow: 0 0 5px rgba(0, 255, 255, 0.05);
          transition: box-shadow 0.3s ease-in-out, transform 0.3s ease-in-out;
        }
        .card-shadow-effect:hover {
            box-shadow: 0 0 15px rgba(0, 255, 255, 0.1); /* Slightly more glow on hover */
        }

        /* Pseudo-elements for the L-shape glowing shadow */
        .card-shadow-effect::before,
        .card-shadow-effect::after {
            content: '';
            position: absolute;
            pointer-events: none;
            transition: opacity 0.3s ease-in-out;
            opacity: 0; /* Hidden by default */
            z-index: -2; /* Ensure it's behind the card content */
        }

        /* Bottom part of the L-shadow glow */
        .card-shadow-effect::before {
            bottom: 0;
            left: 0;
            width: 100%;
            height: 10px; /* Height of the glow band */
            box-shadow:
                0 5px 20px 10px rgba(0, 255, 255, 0.5), /* Primary bottom glow */
                0 5px 40px 15px rgba(100, 200, 255, 0.3); /* Secondary wider bottom glow */
        }

        /* Right part of the L-shadow glow */
        .card-shadow-effect::after {
            top: 0;
            right: 0;
            height: 100%;
            width: 10px; /* Width of the glow band */
            box-shadow:
                5px 0 20px 10px rgba(0, 255, 255, 0.5), /* Primary right glow */
                5px 0 40px 15px rgba(100, 200, 255, 0.3); /* Secondary wider right glow */
        }

        .card-shadow-effect:hover::before,
        .card-shadow-effect:hover::after {
