...
const containerVariants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.1 } }, // Reduced stagger time
};

const cardVariants = {
  hidden: { opacity: 0, y: 20 }, // Simplified animation
  visible: {
    opacity: 1,
    y: 0,
  ...
  return (
    <div className="font-sans">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${photo0.src})` }}
      >
        ...
        >
          ...
          <h1 className="text-5xl font-extrabold mb-3 tracking-wide drop-shadow-lg text-white">Premium Grooming Products</h1>
          <div className="h-1 w-32 bg-gradient-to-r from-yellow-400 to-yellow-600 mx-auto rounded-full mb-4"></div>
          <p className="max-w-2xl mx-auto text-lg opacity-90 text-white">
      ...
      {/* Products Grid */}
      <div className="relative bg-gradient-to-br from-gray-50 to-gray-200 py-20 px-6" ref={inViewRef}>
        <motion.div
        ...
        >
          {products.map((product) => (
            <motion.div
              key={product.id}
              className="relative w-full aspect-[3/4] shadow-xl cursor-pointer bg-white border border-gray-200 group card-with-gradient-border"
      ...
      <AnimatePresence>
        {selectedProduct && (
          <motion.div className="fixed inset-0 bg-gray-200/90 backdrop-blur-lg flex items-center justify-center p-4 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div
              className="relative bg-white text-gray-900 p-8 max-w-2xl w-full mx-auto shadow-2xl overflow-hidden border border-gray-200"
